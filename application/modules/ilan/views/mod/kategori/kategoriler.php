...
<script type="text/javascript">
  ...
  function itemDelete(id,token){
      ...
      $.ajax({
         type:'POST',
         url:'<?php echo base_url().url_title($this->meslekname) ?>/admin/ilanlar/kategoriler/delete/'+id+'?token='+token,
         dataType:'json',
         success:function(respond){  
            if(respond.success=="true"){
               $("#result").html(resultFormat("success","<?php echo lang("Seçmiş olduğunuz kategori silinmiştir.") ?> "));
               setTimeout("window.location='<?=base_url().url_title($this->meslekname)?>/admin/ilanlar/kategoriler'",2000);
...
<div id="rightPanel">
   <div id="result"></div>
   <div class="block">
      <div class="head"><?php echo lang("İlan Kategorileri") ?></div>
      <table id="example" width="100%" class="display">
         ...
         <tbody>
            ...
            <tr id="row_<?php echo $row->id ?>">
               ...
               <td><a href="<?php echo base_url().url_title($this->meslekname) ?>/admin/ilanlar/kategoriler/edit/<?php echo $row->id ?>?token=<?php echo setToken($row->id)?>"><img src="images/icons/bookmark--pencil.png"/></a></td>
