...
<script type="text/javascript">
   ...
   function formSubmit(){
      if( $("#name").val()==""){
         $("#result").html(resultFormat("warning","<?php echo lang("Makale Başlığı Giriniz!") ?>"));   
         return false;
      }
      if( $("#lang").val()==""){
         $("#result").html(resultFormat("warning","<?php echo lang("Makale Dilini Giriniz!") ?>"));   
         return false;
      } 
      if( $("#description").val()==""){
         $("#result").html(resultFormat("warning","<?php echo lang("Makale Açıklaması Giriniz!") ?>"));   
         return false;
      }
      ...
      $.ajax({
         type:'POST',
         url:'<?php echo base_url().url_title($this->meslekname) ?>/admin/makaleler/save',
         data: $("#pageForm").serialize(),
         dataType:'json',
         success:function(respond){  
            if(respond.success=="true"){
               $("#result").html(resultFormat("success","<?php echo lang("Makale Başarıyla Eklenmiştir.") ?> "));
               setTimeout("window.location='<?=base_url().url_title($this->meslekname)?>/admin/makaleler'",2000);

            }else if (respond.success=="false") {
               $("#result").html(resultFormat("alert","<?php echo lang("Makale Eklenemedi!") ?>"));
            }
         }
      });  
   }
...
<div id="rightPanel">
   <div class="block">
      <div class="head"><?= lang("Makale Ekle") ?></div>
