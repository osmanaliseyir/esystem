...
<script type="text/javascript">
   ...
   function itemDelete(id,token){
      ...
      $.ajax({
         type:'POST',
         url:'<?php echo base_url().url_title($this->meslekname) ?>/admin/haberler/delete/'+id+'?token='+token,
         dataType:'json',
...
<div id="rightPanel">
   <div id="result"></div>
   <div class="block">
      <div class="head"><?php echo lang("Haberler") ?>
        <span class="right" style="padding:4px;"><?php echo form_open(base_url().url_title($this->meslekname)."/admin/haberler/", array("name" => "searchForm", "id" => "searchForm", "method" => "get")); ?>
        <?php echo form_dropdown("active",$this->durum,(isset($_GET["active"]))? $_GET["active"]: "" ) ?>
        <input type="input" name="q" id="q" value="<?= isset($_GET["q"]) ? $_GET["q"] : "" ?>" style="border:1px solid #666666; padding:4px; width:214px; font-size:11px;" placeholder="Haberlerde arama yapın"/>         
      ...
      <table id="example" width="100%" class="display" style="font-size:11px;">
         ...
         <tbody>
            ...
            <tr id="row_<?php echo $row->id ?>">
               ...
               <td><a href="<?php echo base_url().url_title($this->meslekname) ?>/admin/haberler/edit/<?php echo $row->id ?>?token=<?php echo setToken($row->id)?>"><img src="images/icons/bookmark--pencil.png"/></a>
   ...
   <?php if (count($data) > 0) { ?>
            <div class="pagination">
                <div style="width:200px; font-size:11px; color:#666666; float:left;">
                    <?php echo lang("Toplam") . " <b style='color:red'>" . $this->mod_haber_model->total_rows . "</b> " . lang("adet") . " " . lang("haber kaydı bulundu!") ?>
                </div>
                <div style="float:left; text-align: right; width:443px;">
                    ...
                    $config['total_rows'] = $this->mod_haber_model->total_rows;
                    $config['per_page'] = 20;
